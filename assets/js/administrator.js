$(".delete-administrator").click(function(e){
    let id = $(this).attr('data-id');
    $("#delete-dialog .yes").click(function(){
        AJAXRequester('DELETE', 'administrator/'+id, null, null, 'wrapper').then(res => {
            if(res.data){
...
$(window).scroll(function(e){
    if(endPage() && !stopIncrease){
        AJAXRequester('GET', 'administrator?pagination='+pagination, null, null, 'pagination').then(res => {
            if(res){
                $('#list ul').append(res);
                nextPage();
            }else
